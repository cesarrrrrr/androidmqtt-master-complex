...
public class MqttConfig {
    ...
    private static String userID =Constants.MQTT_USERNAME;
    private static String passWord =Constants.MQTT_PASSWORD;
    private static String clientID =Constants.MQTT_CLIENTID;
    private static String timeout =Constants.MQTT_TIMEOUT;
    private static String keepalive=Constants.MQTT_KEEPALIVE ;
    private static boolean cleansession=Constants.MQTT_SeESSION ;
    ...
    private MqttConnectOptions getOptions(){
        ...
        if (!TextUtils.isEmpty(timeout)){
            if (Integer.parseInt(timeout)>0){
                options.setConnectionTimeout(Integer.parseInt(timeout));//设置连接超时时间
            }else {
        ...
        if (!TextUtils.isEmpty(timeout)){
            if (Integer.parseInt(keepalive)>0){
                options.setKeepAliveInterval(Integer.parseInt(keepalive));//设置设置连接超时时间
            }else {
        ...
        options.setCleanSession(cleansession); //默认
        return options;
    }
